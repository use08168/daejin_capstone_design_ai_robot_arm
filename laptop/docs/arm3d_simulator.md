...
> 📷 **사진 자리:** 참조 로봇팔 설계도 — `docs/assets/01-reference-design.jpg` (추가 예정)
> 📷 **사진 자리:** 실물 조립 완료 사진 — `docs/assets/02-real-arm.jpg`
...
> 📷 **사진 자리:** 결합 중 원통/평면 선택 화면 — `docs/assets/03-face-mate.jpg`
> 📷 **사진 자리:** 6관절 조립 완료 + 고정 연결선 — `docs/assets/04-assembled.jpg`

이렇게 만든 트윈은 **실측 STL 기반**이라, 시뮬레이션 자세가 곧 실물 자세에 대응한다(아래 **§6 제어 모드**).
사진 파일을 `docs/assets/`에 넣으면 위 자리에 링크가 연결된다 → [assets/README](assets/README.md).
