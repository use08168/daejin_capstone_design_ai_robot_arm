...
- **모터** channel · us0(0°µs) · us180(180°µs) — 각도→펄스 변환 파라미터(관절별 실측값 입력, → [1_servo_calibration.md](../../arduino/docs/1_servo_calibration.md)).
...
## 9. 안전 검증 — 충돌 감지 · C-space · 능동학습 (제어 모드)

디지털 트윈을 **안전 사전검증**에 쓰는 기능을 제어 모드에 내장했다. 원리·결과·그림은 **[6_digital_twin_safety.md](6_digital_twin_safety.md)** 에 상세히 있고, 여기서는 시뮬레이터 UI만 정리한다.

| UI (제어 모드) | 동작 |
|------|------|
| **충돌 HUD**(우상단) | 매 프레임 STL 정점 vs 바닥(y=0)·그리퍼 vs 비인접 링크 검사 → `바닥 여유 mm · 안전/위험/충돌` + 충돌점 마커. `충돌` 토글. |
| **🗺 격자(J2·J3·J4)** | 전 조합(5°)을 시뮬에서 회전·충돌기록 → `cspace_grid.csv` (히트맵용) |
| **🎲 랜덤(J2~J6)** | N개 무작위 자세 충돌기록 → `cspace_rand.csv` (고차원 학습용, J1은 충돌 불변→고정) |
| **🏷 질의 라벨링(CSV)** | Python(`cspace_active_loop.py`)이 고른 불확실 자세 `cspace_query.csv`를 업로드 → 시뮬로 라벨 → `cspace_labeled.csv` (능동학습 루프) |

- **안전 마진**은 콜드스타트로 측정한 **sim-real gap(30mm)** 에 연동 → "시뮬 안전 ⇒ 실물 안전" 보증. ([5_coldstart_procedure.md](5_coldstart_procedure.md))
- 이 데이터로 학습한 **충돌예측 모델(0.8µs/질의)** 이 AI 명령의 **안전 검증층** 역할 — LLM 할루시네이션을 실물 실행 전에 차단.
- 충돌 코어(`_collideOnce`)를 HUD·스윕·라벨링이 공유해 일관성 보장.
...
- **단방향 연동**: 실물 서보에 위치 피드백이 없어 3D→실물만. (마커 추적으로 보정은 [15_aruco_markers.md](15_aruco_markers.md) 단계 ⑥에서)
